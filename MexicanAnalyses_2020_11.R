...
mexican <- read.csv(file= "ReuseDataMexican_2019_05_10.csv", header = TRUE, sep=",")
mexican <- mexican %>%
  dplyr::select(-c(X))
summary(mexican)

# Data from Session 1: 
entrain <- read.csv(file= "EntrainDataMexican_2019_05_10.csv", header = TRUE, sep=",")
entrain <- entrain %>%
  dplyr::select(-c(X)) 
summary(entrain)
...
conditions_entrain <- entrain %>% 
  ...
  dplyr::summarise(no_rows = length(partner) / 21)
#ingroup = 80 vs outgroup = 81

alignbyitem <- entrain %>%
  dplyr::group_by(target, partner, freq1) %>%
  mutate(FirstPartner = ifelse(as.character(partner) %in% c("in"), "IngroupPartner", "OutgroupPartner")) %>%
...
describe(alignbyitem_spread$freq1)
...
alignbyitem2 <- entrain %>%
  ...
  mutate(FirstPartner = ifelse(as.character(condition) %in% c("MM", "MA"), "IngroupPartner", "OutgroupPartner"), 
         SecondPartner = ifelse(as.character(condition) %in% c("MM", "AA"), "Same", "Different")) %>%
  ...
  dplyr::summarise(mean = mean(alignbyitem), 
                   ...
                   N = n(),
                   se = sd/sqrt(n()))
alignbyitem2[5,]=list("Pretest", "Pretest", 4, 6.06, 21, 1.32)
...
plot1 <- ggplot(subset(alignbyitem2, FirstPartner != "Pretest"),
  ...
  scale_x_discrete( breaks = c("IngroupPartner", "OutgroupPartner"), 
                    labels=c("In-community", "Out-Community"))+
...
entrain <- entrain %>%
  ...
  dplyr::mutate(CenPartner = ifelse(partner %in% c("in"), -1, 1), 
                SecondPartner = ifelse(condition %in% c("AA", "MM"), "Same", "Different"), 
                CenSecond = ifelse(SecondPartner %in% c("Same"), -1, 1))
summary(entrain)


condition_entrain_sing <- glmer(aligncode ~ CenPartner + CenSecond + CenPartner:CenSecond + (1+CenPartner + CenSecond + CenPartner:CenSecond |target) + (1|ID), 
                           data = entrain, family=binomial, 
                           na.action = na.omit, 
                           glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(condition_entrain_sing)

condition_entrain_sing2 <- glmer(aligncode ~ CenPartner + CenSecond + CenPartner:CenSecond + (1+CenPartner + CenSecond |target) + (1|ID), 
                                data = entrain, family=binomial, 
                                na.action = na.omit, 
                                glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(condition_entrain_sing2)

condition_entrain_sing3 <- glmer(aligncode ~ CenPartner + CenSecond + CenPartner:CenSecond + (1+CenPartner |target) + (1|ID), 
                                 data = entrain, family=binomial, 
                                 na.action = na.omit, 
                                 glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(condition_entrain_sing3)

condition_entrain <- glmer(aligncode ~ CenPartner + CenSecond + CenPartner:CenSecond + (1 |target) + (1|ID), 
                                 data = entrain, family=binomial, 
                                 na.action = na.omit, 
                                 glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
...
BIC(condition_entrain2) #3210.198
...
BIC(condition_entrain_null) #3203.061
...
mexican$ID <-as.factor(mexican$ID)
mexican$target <-as.factor(mexican$target)
mexican$reuse <-as.factor(mexican$reuse)
mexican$switch <-as.factor(mexican$switch)
mexican$first <-as.factor(mexican$first)
mexican$second <-as.factor(mexican$second)
mexican$R1 <-as.factor(mexican$R1)

reuse_ID = na.omit(mexican) %>%
  dplyr::group_by(ID) %>%
  dplyr::summarise(entrained1 = sum(R1=="1" & R2=="1")*100/20) %>%
  dplyr::summarise(entrained1.m = mean(entrained1),
                   entrained1.sd = sd(entrained1))
reuse_ID

reuse_target = na.omit(mexican) %>%
  dplyr::group_by(target) %>%
...
reuse_FirstSwitch = na.omit(mexican) %>%
  dplyr::filter(R1==1) %>%
...
reuse_entrained_graph <- ggplot(reuse_FirstSwitch, aes(x = first, y = Reuse.m, fill= switch)) +
  ...
  theme(legend.position=c(0.5, 0.95),
        ...
        strip.text = element_text(color="black", size=16)) +
  ylim(0,100) +
  scale_x_discrete(labels = c("in" = "In-Community","out" = "Out-Community")) +
  scale_fill_manual(values=c("grey70", "grey30"), 
                    name  ="Second Partner's \nCommunity",
                    breaks=c("no", "yes"),
                    labels=c("Same as\nfirst partner", "Different from\nfirst partner"))
...
Interaction <- glmer(reuse ~ CenFirst + CenSwitch + CenFirst:CenSwitch + (1+CenFirst + CenSwitch + CenFirst:CenSwitch|target) + (1|ID), 
                                data = subset(data, R1==1), family=binomial, 
                                na.action = na.omit, 
                                glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(Interaction)

# There's an interaction again

# subset analysis
SwitchInsiders_s <- glmer(reuse ~ CenSwitch + (1+CenSwitch|target) + (1|ID), 
                        data = subset(data, R1==1 & first=="in"), family=binomial, 
                        na.action = na.omit, 
                        glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(SwitchInsiders_s)
...
SwitchOutsiders<- glmer(reuse ~ CenSwitch + (1+CenSwitch|target) + (1|ID), 
