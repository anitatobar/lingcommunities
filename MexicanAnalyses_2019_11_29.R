setwd("C:/Users/s1552951/Dropbox/R/EnglishCoordination/Mexican")
...
condition_entrain <- glmer(aligncode ~ CenPartner*CenSecond + (1|target) + (1|ID), 
                           data = entrain, family=binomial, 
                           na.action = na.omit, 
                           glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(condition_entrain)

condition_entrain2 <- glmer(aligncode ~ CenSecond + (1|target) + (1|ID), 
                            data = subset(entrain, partner== "in"), family=binomial, 
                            na.action = na.omit, 
                            glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
...
reuse_entrained_graph <- ggplot(reuse_FirstSwitch, aes(x = first, y = Reuse.m, fill= switch)) +
  ...
  geom_line(aes(group = switch), size=.5, position=position_dodge(.3)) +
  ...
  theme(legend.position=c(0.6, 0.95),
        ...
        strip.text = element_text(color="black", size=12)) +
  ylim(0,100) +
  scale_x_discrete(labels = c("in" = "Insider","out" = "Outsider")) +
  scale_fill_manual(values=c("grey70", "grey30"), 
                    name  ="Second Partner's \nCommunity",
                    breaks=c("no", "yes"),
                    labels=c("Same", "Different"))
...
Interaction_singlefit1 <- glmer(reuse ~ CenFirst*CenSwitch + (1+CenFirst+CenSwitch|target) + (1|ID), 
                                data = subset(data, R1==1), family=binomial, 
                                na.action = na.omit, 
                                glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(Interaction_singlefit1)

Interaction_singlefit2 <- glmer(reuse ~ CenFirst*CenSwitch + (1+CenFirst|target) + (1|ID), 
                                data = subset(data, R1==1), family=binomial, 
                                na.action = na.omit, 
                                glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
...
Interaction_singlefit3 <- glmer(reuse ~ CenFirst*CenSwitch + (1+CenSwitch|target) + (1|ID), 
                                data = subset(data, R1==1), family=binomial, 
                                na.action = na.omit, 
                                glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(Interaction_singlefit3)

Interaction <- glmer(reuse ~ CenFirst*CenSwitch + (1|target) + (1|ID), 
                     data = subset(data, R1==1), family=binomial, 
                     na.action = na.omit, 
                     glmerControl(optimizer='bobyqa', optCtrl=list(maxfun=400000)))
summary(Interaction)
